  ...
  return (
    <Container sx={{ marginTop: 5}}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", marginBottom: 2 }}>
        <Typography variant="h2" sx={{ marginRight: 3 }} style={{ lineHeight: 0.75 }}>{data.title}</Typography>
        <Button
          variant="contained"
          color="primary"
          href={`slack://user?team=T05JTJTBSTS&id=${data.slac_id}`}
          target="_blank"
      ...
      <Typography color="text.secondary" gutterBottom sx={{ marginTop: 1}}>
        {data.user_name},{" "}
        {new Date(data.task_date).toLocaleDateString()}
      </Typography>
      <hr/>
      ...
      <Card elevation={ 0 } sx={{ border: 1, borderColor: "#DDDDDD", borderRadius: 1, marginTop: 1, padding: 2 }}>
        ...
        <Typography variant="body2" sx={{marginTop: 2, marginBottom: 2}}>{data.task_detail}</Typography>
        <hr/>
        <Box sx={{ height: 20}} />
        <Box sx={{ borderLeft: 5, paddingTop: 1, paddingBottom: 1, borderColor: "#BBBBBB" }}>
          <Typography variant="h5" sx={{ marginRight: 3, marginLeft:1 }} style={{ lineHeight: 0.5 }}>
        ...
        <Typography variant="body2" sx={{marginTop: 2}}>{data.concern_desc}</Typography>
      </Card>
    </Container>
  );
}
